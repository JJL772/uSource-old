...
def configure(conf):
        ...
        if conf.env.DEST_OS is 'android' or conf.env.DEST_CPU in ['aarch32', 'aarch64']:
        ...
        conf.options.USE_SSE    = True if conf.env.DEST_CPU in ['x86', 'x86_64'] else False 
        conf.options.USE_SSE42  = True if conf.env.DEST_CPU in ['x86', 'x86_64'] else False
        conf.options.USE_AVX    = True if conf.env.DEST_CPU in ['x86_64'] else False 
        conf.options.USE_NEON   = True if conf.env.DEST_CPU in ['aarch32', 'aarch64'] else False 
        opts = list()
        ...
        compiler_c_cxx_flags = {
                'common': {
                        ...
                        'gcc': ['-g', '-fvisibility=hidden', '-fpermissive']
                },
                'fast': {
