...
class CEngineStudio : public IEngineStudio
{
public:
	...
	virtual const char* GetName() { return "CEngineStudio001"; };
	virtual const char* GetParentInterface() { return IENGINESTUDIO_INTERFACE; }

	virtual void		*Mem_Calloc ( int number, size_t size ) 
	{
		return pfnMod_Calloc(number, size);
	}

	virtual void		*Cache_Check ( struct cache_user_s *c ) 
	{
		return pfnMod_CacheCheck(c);
	}

	virtual void		LoadCacheFile ( const char *path, struct cache_user_s *cu ) 
	{
		pfnMod_LoadCacheFile(path, cu);
	}

	virtual struct model_s	*Mod_ForName ( const char *name, int crash_if_missing ) 
	{
		return pfnMod_ForName(name, crash_if_missing);
	}

	virtual void		*Mod_Extradata ( struct model_s *mod ) 
	{
		return pfnMod_StudioExtradata(mod);
	}

	virtual struct model_s	*GetModelByIndex ( int index ) 
	{
		return pfnModelHandle(index);
	}

	virtual struct cl_entity_s * GetCurrentEntity ( void ) 
	{
		return pfnGetCurrentEntity();
	}

	virtual struct player_info_s *PlayerInfo ( int index ) 
	{
		return pfnPlayerInfo(index);
	}

	virtual struct entity_state_s *GetPlayerState ( int index ) 
	{
		return R_StudioGetPlayerState(index);
	}

	virtual struct cl_entity_s * GetViewEntity ( void ) 
	{
		return pfnGetViewEntity();
	}

	virtual void		GetTimes ( int *framecount, double *current, double *old ) 
	{
		pfnGetEngineTimes(framecount, current, old);
	}

	virtual struct cvar_s	*GetCvar ( const char *name ) 
	{
		return pfnGetCvarPointer(name);
	}

	virtual void		GetViewInfo ( float *origin, float *upv, float *rightv, float *vpnv ) 
	{
		pfnGetViewInfo(origin, upv, rightv, vpnv);
	}

	virtual struct model_s	*GetChromeSprite ( void ) 
	{
		return R_GetChromeSprite();
	}

	virtual void		GetModelCounters ( int **s, int **a ) 
	{
		pfnGetModelCounters(s, a);
	}

	virtual void		GetAliasScale ( float *x, float *y ) 
	{
		pfnGetAliasScale(x, y);
	}

	virtual float		****StudioGetBoneTransform ( void ) 
	{
		return pfnStudioGetBoneTransform();
	}

	virtual float		****StudioGetLightTransform ( void ) 
	{
		return pfnStudioGetLightTransform();
	}

	virtual float		***StudioGetAliasTransform ( void ) 
	{
		return pfnStudioGetAliasTransform();
	}

	virtual float		***StudioGetRotationMatrix ( void ) 
	{
		return pfnStudioGetRotationMatrix();
	}

	virtual void		StudioSetupModel ( int bodypart, void **ppbodypart, void **ppsubmodel ) 
	{
		return R_StudioSetupModel(bodypart, ppbodypart, ppsubmodel);
	}
	...
	virtual void		StudioDynamicLight ( struct cl_entity_s *ent, struct alight_s *plight ) 
	{
		R_StudioDynamicLight(ent, plight);
	}

	virtual void		StudioEntityLight ( struct alight_s *plight ) 
	{
		R_StudioEntityLight(plight);
	}

	virtual void		StudioSetupLighting ( struct alight_s *plighting ) 
	{
		R_StudioSetupLighting(plighting);
	}
	...
	virtual void		StudioDrawHulls ( void ) 
	{
		R_StudioDrawHulls();
	}

	virtual void		StudioDrawAbsBBox ( void ) 
	{
		R_StudioDrawAbsBBox();
	}

	virtual void		StudioDrawBones ( void ) 
	{
		R_StudioDrawBones();
	}

	virtual void		StudioSetupSkin ( void *ptexturehdr, int index ) 
	{
		R_StudioSetupSkin((studiohdr_t*)ptexturehdr, index);
	}

	virtual void		StudioSetRemapColors ( int top, int bottom ) 
	{
		R_StudioSetRemapColors(top, bottom);
	}

	virtual struct model_s	*SetupPlayerModel ( int index ) 
	{
		return R_StudioSetupPlayerModel(index);
	}
	...
	virtual int		GetForceFaceFlags ( void ) 
	{
		return R_StudioGetForceFaceFlags();
	}

	virtual void		SetForceFaceFlags ( int flags ) 
	{
		R_StudioSetForceFaceFlags(flags);
	}

	virtual void		StudioSetHeader ( void *header ) 
	{
		R_StudioSetHeader((studiohdr_t*)header);
	}

	virtual void		SetRenderModel ( struct model_s *model ) 
	{
		R_StudioSetRenderModel(model);
	}

	virtual void		SetupRenderer ( int rendermode ) 
	{
		R_StudioSetupRenderer(rendermode);
	}

	virtual void		RestoreRenderer ( void ) 
	{
		R_StudioRestoreRenderer();
	}

	virtual void		SetChromeOrigin ( void ) 
	{
		R_StudioSetChromeOrigin();
	}

	virtual int		IsHardware ( void ) 
	{
		return pfnIsHardware();
	}
	...
	virtual void		GL_SetRenderMode ( int mode ) 
	{
		::GL_SetRenderMode(mode);
	}

	virtual void		StudioSetRenderamt ( int iRenderamt ) 
	{
		R_StudioSetRenderamt(iRenderamt);
	}

	virtual void		StudioSetCullState ( int iCull ) 
	{
		R_StudioSetCullState(iCull);
	}

	virtual void		StudioRenderShadow ( int iSprite, float *p1, float *p2, float *p3, float *p4 ) 
	{
		R_StudioRenderShadow(iSprite, p1, p2, p3, p4);
	}

};

EXPOSE_INTERFACE(CEngineStudio);
