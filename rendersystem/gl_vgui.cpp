...
void GAME_EXPORT VGUI_UploadTexture( int id, const char *buffer, int width, int height )
{
	...
	g_textures[id] = GL_LoadTextureInternal( texName, &r_image, (texFlags_t)TF_IMAGE );
}
...
void GAME_EXPORT VGUI_CreateTexture( int id, int width, int height )
{
	...
	g_textures[id] = GL_LoadTextureInternal( texName, &r_image, (texFlags_t)(TF_IMAGE|TF_NEAREST) );
	g_iBoundTexture = id;
}
